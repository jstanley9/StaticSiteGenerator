...
def copy_templates_to_target(static_path, target_directory):
    print(f'** copy {static_path} contents to {target_directory}')
    ...
    for path in os.listdir(static_path):
        ...
        if os.path.isdir(source_path):
            ...
            print(f'** copy {source_path} to directory {target_directory}')
            shutil.copy(source_path, target_directory)

def clear_directory(path):
    print(f'** clear_directory:: {path}')
    for name in os.listdir(path):
        file_title = os.path.join(path, name)
        if os.path.isdir(file_title):
            clear_directory(file_title)
            print(f'** remove directory: {file_title}')
            os.rmdir(file_title)
        else:
            print(f'** remove:: {file_title}')
